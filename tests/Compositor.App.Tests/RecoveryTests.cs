...
public class RecoveryTests
{
    ...
    {
        var folder = Path.Combine(Path.GetTempPath(), "compositor-recovery-" + Guid.NewGuid().ToString("N"));
        try
        {
            ...
            var written = File.GetLastWriteTimeUtc(Directory.GetFiles(folder, "*.compositor")[0]);
            await recovery.Save([session]);
            Assert.Equal(written, File.GetLastWriteTimeUtc(Directory.GetFiles(folder, "*.compositor")[0])); // Unchanged since: not rewritten.
            ...
            var restored = Compositor.IO.ProjectFile.Load(abandoned.ProjectPath);
