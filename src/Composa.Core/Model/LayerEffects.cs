...
/// <summary>A soft light spreading out from the layer's edges on every side, behind its pixels.</summary>
public sealed record OuterGlowEffect
{
    public const double MaxSize = 500;
    public bool Enabled { get; init; } = true;
    /// <summary>How far the glow reaches, in layer pixels.</summary>
    public double Size { get; init; } = 20;
    public uint Color { get; init; } = 0xFFFFFFFF;
    public double Opacity { get; init; } = 0.75;

    public OuterGlowEffect Clamped() => this with
    {
        Size = double.IsFinite(Size) ? Math.Clamp(Size, 0, MaxSize) : 20,
...
public sealed record LayerEffects
{
    ...
    public int Margin()
    {
        ...
        // The glow is a Gaussian of half the size; three sigmas out it has faded to nothing.
        if (visible.OuterGlow is { } glow && glow.Opacity > 0) margin = Math.Max(margin, glow.Size * 1.5);
