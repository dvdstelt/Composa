...
/// <summary>
/// Reads projects saved by Composa for macOS: a <c>.comp</c> package, which on Linux is an ordinary folder holding
...
public static class MacProject
{
    ...
    public static Document Load(string folder)
    {
        ...
        if (!File.Exists(manifestPath)) throw new InvalidDataException("This folder is not a Composa project: it has no manifest.json.");
        ...
        if (Text(root, "format") != Format) throw new InvalidDataException("This is not a Composa project.");
